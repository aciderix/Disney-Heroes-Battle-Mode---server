...
> 3. **HERO_FILTERS** (là où l'auto-pilote calait) : menu HEROES → HeroListScreen (3 héros niv.2 W + héros verrouillé) → bulle tuto « Hero Filters… » → **FILTER** (`FILTER_BUTTON`) → **HERO FILTERS window** (catégories GENERAL/EFFECTS/±STATS/TALENTS, Sources, Team…) → navigation catégories + toggles → **acte HERO_FILTERS 1→4 = COMPLET (step=maxStep=4)**. Le blocage historique du pilote (getPointers vide headless) est **résolu par le contrôle manuel/semi-auto** (les commandes `drive`/`center` du (g3) marchent : log `[semiauto] center → tap`).
**⇒ Le serveur alimente/valide/persiste tous les écrans post-tuto testés (nom, sign-in, filtres) sur le client 100 % d'origine.** Checkpoint : `server/data/dh-snapshot-manualtests-0719.db` (nom changé + or 226M + HERO_FILTERS complet). **Reste à explorer** (prochains écrans) : ITEMS, QUESTS, MEDALS, MAILBOX, EVENTS, promotion de héros (PROMOTE_HERO), UNLOCK_HERO (Vanellope), campagne au-delà de 1-7. Fichiers : `MEMORY.md`, `JOURNAL.md`.
