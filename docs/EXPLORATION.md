...
| 19 | FRANCHISE_TRIALS / TEAM_TRIALS | 55 | ⬜ | même sous-système `DifficultyModeHelper` que PORT (rangée 21) ; rotation par `TrialsHelper.*_OPEN_DAYS` (défaut du jeu) — **ouvrable hors planning via `AdminEvents --open` (moteur SPECIAL_EVENTS g131)**. Reste : jouer en jeu (mêmes handlers combat/raid que PORT). |
