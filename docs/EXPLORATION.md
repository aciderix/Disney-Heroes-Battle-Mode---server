...
| 2-3 | RANKINGS / FIGHT_PIT (arène PvP) | 10 | ⚠️ | **FEATURE PvP MULTI-ÉTAPES** — établi par les faits : `GET_ARENA_RANKINGS`/`GET_PLAYER_RANKINGS`, `ArenaInfo{season, yourLeague, topLeague}`, `ArenaRow` (adversaires), `ArenaAttack{defendingUserID}` (combat PvP), saisons/promotions/démotions/récompenses hebdo. **Bute sur le mur PvP** : les adversaires sont des **snapshots d'autres joueurs** → **inexistants sur un serveur mono-compte hors-ligne**. Rendre l'arène jouable en solo = **sous-système d'adversaires synthétiques** (placement + génération d'équipes de défense IA + rangs) = feature dédiée. À décider avec l'utilisateur (cf. journal). |
| 4 | ELITE_CAMPAIGN | 11 | ⬜ | |
