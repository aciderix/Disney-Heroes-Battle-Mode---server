...
| 4 | ELITE_CAMPAIGN | 11 | 🟢 | **Serveur COMPLET + prouvé headless ; vérif EN JEU bloquée par l'ère de contenu (pré-existant, hors scope raid).** PLAY = `CampaignAttack{ELITE}`→`recordCampaignAttack` (déjà géré, route `GameMode.ELITE_CAMPAIGN`). RAID = **nouveaux handlers `RaidCampaign`/`RaidAllCampaign`** (`recordRaidCampaign`/`recordRaidAllCampaign`) rejouant fidèlement la séquence client (bytecode `CampaignPreviewScreen`/`RaidTicketOutcomeWindow`) : `chargeForRaid` (débit RAID_TICKET+énergie, `useTickets=!VIP RAID_WITHOUT_TICKETS`, anti-triche tickets/énergie/VIP RAID_10/chances ELITE/3★-ou-CAMPAIGN_UNLOCKED) + `recordRaidOutcome` (progression `totalWins/dailyUses+=raidCount`, gold, XP-items, crédit loot CLIENT §4bis). Prouvé `EliteCampaignRaidTest` (raid NORMAL 1-1 : énergie -6, ticket -1, or +340, totalWins +1, loot +5, persiste). **BLOCAGE vérif en jeu (§8, faits)** : le compte de test TL65 résout l'ère **R102** (`maxHeroLevel=565`) → `recordOutcome`/`addHeroEXP` **déborde l'entier** (`ClientErrorCode.ERROR`) = le chantier **ÈRE DE CONTENU** documenté (SHIMS : stamina 39,96 M / TL 565), PRÉ-EXISTANT, affecte TOUT combat de campagne à haut TL. Débloquer l'élite naturellement exige TL11 + progression multi-chapitres (3★ tout le chapitre 1 → TL 4 seulement, élite non débloquée). Outil DEV `EliteSetup` (3★ chapitre 1 + tickets, marche sur comptes bas-TL sans débordement). **À finir en jeu** une fois l'ère de contenu ancrée (ou compte bas-TL avec élite). |
