...
| 19 | FRANCHISE_TRIALS / TEAM_TRIALS | 55 | 🚧 | **TEAM_TRIALS_BLUE ✅ VÉRIFIÉ EN JEU (g134)** : `DifficultyMode` réutilisant TOUTE l'infra PORT (`getOpenDays`/MODES_OPEN/`ModePreviewScreen`/`recordOutcome`) — ouvert via `AdminEvents --open`, joué de bout en bout (BLUE TEAM → combat → REWARDS → cooldown persisté). TEAM_TRIALS_{RED,YELLOW} = MÊME code (mode=paramètre). **Reste** : SPOTLIGHT (conso `spotlightTrialUses`) + EVENT/FRANCHISE (nouveau `TrialEventInfo` : `GetTrialEventData`/`TrialEventAttack`/subtrials/gating/complétion patched heroes). Cf. `docs/FRANCHISE_TRIALS.md` §11-12. |
