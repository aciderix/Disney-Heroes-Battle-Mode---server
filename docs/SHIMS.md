...
| **`ServerUser.recordCampaignAttack`** (combat de campagne) | ⚠️ **RÉEL** (cœur) **+ PARTIEL** (bords) | Exécute la logique AUTORITATIVE du jeu `CampaignHelper.recordOutcome(user, user, CampaignLevel.of(mode, ch, lvl), outcome, stars, stagesCleared, loot, mem, base.attackers, base.defenders, snapshot)` : **consomme la stamina** (`getStaminaCost`+`chargeUser`), **donne loot/gold/XP** (`giveLoot`/`giveGold`/`giveTeamXP`), **met à jour la progression** (`ICampaignLevelStatus`), puis resync + persiste. Fire-and-forget (aucun listener client pour `CampaignAttack` au bytecode). Vérifié (`server/smoke/CampaignAttackTest`) : NORMAL 1-1 WIN → **énergie -6 (120→114), or +340, niveau à 3★**. **PARTIEL** : (1) `SpecialEventSnapshot.NONE` → **aucun bonus d'évènement** appliqué (headless, pas d'évènement live) ; (2) l'`outcome`/`stars` sont **ceux du client** (combat client-autoritatif comme dans le jeu d'origine — pas de re-simulation serveur) ; (3) contrat « fire-and-forget » **à reconfirmer** quand le pilote atteindra le combat en jeu. |
