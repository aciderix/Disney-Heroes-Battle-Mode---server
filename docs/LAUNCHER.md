...
| **C2a-4** **build** depuis l'APK | `POST /build/start {apkPath, target=server\|client\|apk, outDir, full}` ; `GET /build/status` | encapsule `decompile/reframe/extract` + build + **PACKAGING clé-en-main** (bundle autonome lançable). **C2a-4 ✅ cible SERVER** (données + reframe ; `BuildDataGenTest`). **C2a-4-pkg ✅** (packaging serveur autonome : jars+classes+`content_server.py`+`game-data`+`run.sh/.bat` ; `ServerBundleTest` lance le bundle HORS dev). **Reste** : **C2a-4b** (build CLIENT PC Win/Linux : `game-logic-framed`+assets+natifs+gradle → exécutable), **C2a-4c/APK** (patch APK mobile — ULTÉRIEUR : intégrer découverte/redirection serveur). |
