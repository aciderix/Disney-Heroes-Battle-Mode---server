...
| **C2a** launcher-core (daemon HTTP local Java) | 🚧 | **C2a-1 ✅** identité (`LauncherLoginTest` 11) ; **C2a-2 serveurs ✅** (`LauncherServersTest` 12) ; **C2a-2 play ✅ VÉRIFIÉ EN JEU strict** (g191) : compte mnémonique authentifié (challenge→sign→register) → `content_server /login` → `/auth/mint` billet nominatif → `LoginServer` STRICT **accepte** `connexion ← compte 7966…` → BootData → **hub PEUPLÉ rendu** (spine jni) ; cas négatif prouvé : userID non authentifié → mint **401** → billet vide → **rejet**. **Compte NEUF → TUTO D'INTRO lancé** (g192, `intro.png`) : userID frais → `IntroTutorialActV2` (cinématique Ralph/Vanellope + combat scripté) ⇒ nouveau joueur joue normalement ; **joueur avancé reprend** (hub peuplé `strict2.png`). Hook userID **mode-aware** (aucune modif jeu §1) : PERMISSIF = `BuildOptions.TEST_USER_ID` au boot (login UNIQUE) ; STRICT = re-login `GameMain.startInitialLogin(long,int)` (`dh.userid.relogin=1` ; `switchUserAccount`→restart NPE en boucle headless, écarté). **2 correctifs de fidélité** : `DhNet` (GET/DELETE → content en query-string comme libGDX ; sans ça `userID` du `/login` était perdu) + `content_server` (lit `userID` en query). `AuthMintTest` 5. **Reste** : C2a-3 (host), C2a-4 (build APK) |
