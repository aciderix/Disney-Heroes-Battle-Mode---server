...
| E | Tests & intégration APK mobile + **explorateur de serveurs communautaires** | 🚧 **EN COURS** | `docs/SERVER_EXPLORER.md` ; **annuaire brique 1** (fiche serveur SIGNÉE + `/info` vérifiable, g222) **+ brique 2** (table Supabase + Edge Function signée + inscription opt-in + keep-alive, g223, **prouvé LIVE**) LIVRÉES. Reste : brique 3 navigateur launcher PC (lit la table + vérifie `/info` + Jouer), brique 4 patch APK (écran de choix au lancement) ; indép. `GetServers` natif (shard picker) |
