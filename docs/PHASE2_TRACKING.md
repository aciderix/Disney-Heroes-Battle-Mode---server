...
| **C2a** launcher-core (daemon HTTP local Java) | 🚧 | **C2a-1 ✅** identité (`LauncherLoginTest` 11) ; **C2a-2 serveurs ✅** (`LauncherServersTest` 12) ; **C2a-2 play — serveur ✅** : `/auth/mint` billet nominatif (`AuthMintTest` 5) + `content_server` `/login` appelle `/auth/mint` (mode strict `DH_AUTH_URL`). **Reste play** : hook plateforme userID (via API **publique** `GameMain.switchUserAccount(long)` — chemin identifié, aucune modif jeu) + endpoint `/play` + **vérif EN JEU strict**. Puis C2a-3 (host), C2a-4 (build APK) |
