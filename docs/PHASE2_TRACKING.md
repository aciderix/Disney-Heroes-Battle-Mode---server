...
| E | Tests & intégration APK mobile + **explorateur de serveurs communautaires** | 🚧 **EN COURS** | `docs/SERVER_EXPLORER.md` ; **annuaire brique 1 = fiche serveur SIGNÉE + `/info` vérifiable LIVRÉE** (g222, Ed25519 réutilisé §3, `ServerInfoTest`). Reste : brique 2 table Supabase + keep-alive, brique 3 navigateur launcher PC, brique 4 patch APK (écran de choix au lancement) ; indép. `GetServers` natif (shard picker) |
