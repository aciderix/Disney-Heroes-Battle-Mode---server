...
| **C2a** launcher-core (daemon HTTP local Java) | 🚧 | **C2a-1 ✅** (squelette daemon + identité : `/health`, `/identity/generate`, `/identity/login`, `/identity/register` → appelle l'`AuthService` distant) — `LauncherLoginTest` (11 assert.) ; reste C2a-2 (serveurs+play → débloque C1d en jeu), C2a-3 (host), C2a-4 (build APK) |
