...
| C | Front-end joueur (launcher : liste/join serveurs, login) | ✅ **BOUCLÉ (local)** | login mnémonique C1a→C1d ✅ ; **launcher front COMPLET** (7 écrans, endpoints réels+testés, E2E 9/9, CI build 2 OS) ; **panneau Admin 5 domaines vérifié EN JEU** (g215) + **exe FENÊTRÉ distribué** (Tauri, Release `launcher-v0.2.0`) + **admin distant/cloud** (g220). Reste : **son (backend audio non porté)** |
| **D** | Backend/front d'hébergement (panneau opérateur, multi-shard) | ✅ **BOUCLÉ (mono-shard)** | **panneau Admin 5 domaines COMPLET + vérifié EN JEU** (g208-g215 : monitoring, ère, joueurs+audit, events+enums, **modération construite**) via `AdminService` (jeton, bind configurable) + proxy daemon générique + UI Admin. Reste : **multi-shard** (création/isolation, bus inter-shard), observabilité avancée, sauvegarde/restore |
| E | Tests & intégration APK mobile + **explorateur de serveurs communautaires** | 🚧 **EN COURS** | `docs/SERVER_EXPLORER.md` ; **annuaire brique 1** (fiche serveur SIGNÉE + `/info` vérifiable, g222) **+ brique 2** (table Supabase + Edge Function signée + inscription opt-in + keep-alive, g223, **prouvé LIVE**) **+ brique 3** (navigateur communautaire dans le launcher : daemon `/directory` + `/directory/verify`, front, E2E 10/10 + chemin LIVE, g224) LIVRÉES. **Brique 4a** (redirection `ServerType.LIVE` + re-signature, `tools/patch_apk.sh`, **prouvé signé v2/v3**, g226) **+ 4b** (cible « apk » câblée dans le launcher : `/build/start target=apk` + écran Générer host/port, prouvé `ApkBuildProbe`, g227) **+ 4c-1** (fondations écran in-app : Activity `DhServerPicker` compile+dexe, `ServerType.setLive` réassemble, apktool round-trip, g228) **+ 4c-2** (écran de choix injecté dans l'APK : `apk_inject_picker.sh` + câblage launcher, prouvé structurel signé v2/v3, g229) LIVRÉES. **XAPK→universel** (g230 : le base est un base-split d'XAPK → fusion APKEditor + patch chirurgical, universel 158 Mo signé avec libs). Reste : **vérif §8 sur appareil** (générer l'universel depuis le XAPK + installer) ; indép. `GetServers` natif (shard picker) |
| **F** | Tests inter-machines réels (Internet, NAT/TLS, charge) | 🚧 **EN COURS** | **admin distant/cloud livré** (daemon cible `/admin/target` validée + bundle serveur exposable `DH_ADMIN_BIND/TOKEN` + barre front local/distant ; `AdminRemoteTargetTest`) ; **TLS livré (g221)** : `AdminService` HTTPS (keystore PKCS12, `DH_ADMIN_TLS_KEYSTORE/PASS`) + empreinte SHA-256 imprimée au boot + **épinglage** côté launcher (`PinnedTls`, champ front `caFingerprint`) → **jeton chiffré, plus jamais en clair** dès qu'un keystore est fourni (`AdminTlsTest`, régression 176/176). Reste : rate-limit, NAT/UPnP, soak/charge, multi-région |
...
| **C2a** launcher-core (daemon HTTP local Java) | 🚧 | **C2a-1 ✅** identité (`LauncherLoginTest` 11) ; **C2a-2 serveurs ✅** (`LauncherServersTest` 12) ; **C2a-2 play ✅ VÉRIFIÉ EN JEU strict** (g191) : compte mnémonique authentifié (challenge→sign→register) → `content_server /login` → `/auth/mint` billet nominatif → `LoginServer` STRICT **accepte** `connexion ← compte 7966…` → BootData → **hub PEUPLÉ rendu** (spine jni) ; cas négatif prouvé : userID non authentifié → mint **401** → billet vide → **rejet**. **Mode STRICT pour TOUS, LOGIN UNIQUE** (g193) : le boot construit `LoadingScreen(…,0L,0)` (userID=0 en dur → `/login` toujours userID=0 ; le vrai jeu résout par l'appareil). Modèle prod (le launcher fournit l'identité, aucune modif jeu §1) : `BuildOptions.TEST_USER_ID` (hook natif) aligne `ClientInfo.userID` sur le compte authentifié + le **content_server frappe le billet** pour le compte que le launcher authentifie (`DH_MINT_USERID_FILE`) → serveur strict accepte, **1 seul login**. Vérifié EN JEU strict : **NEUF** → `IntroTutorialActV2` = intro (`strictnew.png`) ; **AVANCÉ** → hub peuplé TL 200 (`resume.png`). ⇒ strict identique pour nouveaux et avancés. (`GameMain.startInitialLogin` reste dispo comme repli DEV ; `switchUserAccount`→restart NPE en boucle headless, écarté.) **2 correctifs de fidélité** : `DhNet` (GET/DELETE → content en query-string comme libGDX ; sans ça `userID` du `/login` était perdu) + `content_server` (lit `userID` en query). `AuthMintTest` 5. **C2a-3 host ✅ (headless, g194)** : `dhlauncher.HostManager` + `/host/start,stop,status` (127.0.0.1) — héberge un serveur LOCAL (mirroir exact de `run-online.sh` : `LoginServer`+`AuthService`+`content_server.py` en process). `HostLifecycleTest` 7/7 (start→RUNNING+écoute→TCP OK→stop→fermé). Périmètre MINIMAL (auto-héberger) ; panneau opérateur complet = chantier D, cloud/sécurité internet = chantier F. **C2a-4 build ✅ cible SERVEUR (headless, g195)** : `dhlauncher.BuildManager` + `/build/start {apkPath,target,outDir,full}` + `/build/status` — génère les données+artefacts serveur depuis l'APK (job async vers dossier de sortie, orchestre `extract_game_data.sh`/`decompile`/`reframe`). 3 cibles prévues (util.) : **server** (câblée), **client** PC Win/Linux + **apk** patch mobile = incréments à venir (refus honnête §2). `BuildDataGenTest` 5/5. **C2a-4-pkg ✅** (bundle serveur autonome, `ServerBundleTest`). **C2a-4b ✅ (headless, g198)** : bundle CLIENT PC (BuildManager cible CLIENT + `run-desktop.sh DH_BUILD_ONLY` ; `ClientBundleTest` 9/9, 509 Mo) — lance hors dev + se connecte + MainScreen. **Correctif SourceFile** (ReframeJar) = fix d'un vrai crash 1er lancement. **C2a-4b VALIDÉ EN JEU (g199)** : le prétendu « rendu hub bloqué par l'ENV » de g198 était un **ARTEFACT DE HARNAIS** (capture `DH_SHOT` seule tuée par le `timeout` externe ; process mal détaché) — corrigé avec `setsid`+`-Ddh.shotevery`, le bundle client rend le **hub complet**. **4 combos ✅ EN JEU** (permissif/strict × neuf/avancé). **BUG d'auth strict corrigé** (`LoginServer` : en login unique userID=0, le compte doit être résolu DEPUIS le billet, sinon rejet — sans quoi compte par défaut + auth contournée) ; régression `StrictSingleLoginTest` (7) → **170/170**. Le bundle **RÉUTILISE** les fichiers dev (content_server.py verbatim, mêmes sources serveur, `.so` byte-identiques). **C2a-5 RUNTIME JRE embarqué ✅ (g200)** : `packageRuntime` **jlink** un JRE minimal (~69 Mo, `JRE_MODULES` dont `jdk.crypto.ec`+`jdk.httpserver`) dans `<bundle>/runtime/jre` ; `run.sh`/`run.bat` le préfèrent (repli java système). Vérifié : serveur ET client lourd (hub rendu) sur le JRE embarqué ; `ServerBundleTest` **11/11** (serveur lancé sur JRE embarqué hors dev). ⇒ **port CLIENT PC 100 % autonome**. Limite (§2) : JRE = OS de build (pas de cross-jlink). **Reste zéro-install** : `python3` du content-server (serveur only) ; lanceur natif `.exe`/AppImage ; **patch APK** |
| **C2b** front Tauri+React (6 écrans) + vérif EN JEU strict | ⬜ | après C2a |

Protocole tranché : **daemon HTTP local** (état vivant : serveur hébergé, client lancé, session, progression) vs CLI jetable. Config locale = dossier standard par OS (cf. `LAUNCHER.md` §1bis). Régression : **164/164 verte**.
