...
| E | Tests & intégration APK mobile + **explorateur de serveurs communautaires** | 🚧 **EN COURS** | `docs/SERVER_EXPLORER.md` ; **annuaire brique 1** (fiche serveur SIGNÉE + `/info` vérifiable, g222) **+ brique 2** (table Supabase + Edge Function signée + inscription opt-in + keep-alive, g223, **prouvé LIVE**) **+ brique 3** (navigateur communautaire dans le launcher : daemon `/directory` + `/directory/verify`, front, E2E 10/10 + chemin LIVE, g224) LIVRÉES. **Brique 4a** (redirection `ServerType.LIVE` + re-signature, `tools/patch_apk.sh`, **prouvé signé v2/v3**, g226) **+ 4b** (cible « apk » câblée dans le launcher : `/build/start target=apk` + écran Générer host/port, prouvé `ApkBuildProbe`, g227) LIVRÉES. Reste : 4c écran de choix in-app (injection UI smali, gros morceau) ; indép. `GetServers` natif (shard picker) |
