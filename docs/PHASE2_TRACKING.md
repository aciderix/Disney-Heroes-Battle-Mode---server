...
| E | Tests & intégration APK mobile + **explorateur de serveurs communautaires** | 🚧 **EN COURS** | `docs/SERVER_EXPLORER.md` ; **annuaire brique 1** (fiche serveur SIGNÉE + `/info` vérifiable, g222) **+ brique 2** (table Supabase + Edge Function signée + inscription opt-in + keep-alive, g223, **prouvé LIVE**) **+ brique 3** (navigateur communautaire dans le launcher : daemon `/directory` + `/directory/verify`, front, E2E 10/10 + chemin LIVE, g224) LIVRÉES. **Brique 4a patch APK** (redirection `ServerType.LIVE` + re-signature, `tools/patch_apk.sh`, **prouvé sur 12.1.0 : signé v2/v3**, g226) LIVRÉE. Reste : 4b câbler dans le launcher, 4c écran de choix in-app (injection UI smali) ; indép. `GetServers` natif (shard picker) |
