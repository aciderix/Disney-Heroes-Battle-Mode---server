...
| E | Tests & intégration APK mobile + **explorateur de serveurs communautaires** | 🚧 **EN COURS** | `docs/SERVER_EXPLORER.md` ; **annuaire brique 1** (fiche serveur SIGNÉE + `/info` vérifiable, g222) **+ brique 2** (table Supabase + Edge Function signée + inscription opt-in + keep-alive, g223, **prouvé LIVE**) **+ brique 3** (navigateur communautaire dans le launcher : daemon `/directory` + `/directory/verify`, front, E2E 10/10 + chemin LIVE, g224) LIVRÉES. Reste : brique 4 patch APK (écran de choix au lancement) ; indép. `GetServers` natif (shard picker) |
